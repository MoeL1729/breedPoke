...
test('Dratini hatches with Ember, old saves can recall it without forced replacement',()=>{
 ...
 const old=started(147,20);old.pet.moves=old.pet.moves.filter(m=>m!==52);const restored=new Game(db,JSON.parse(JSON.stringify(old.s)));assert.equal(restored.pet.level,20);assert.ok(!restored.pet.moves.includes(52));restored.recall(52,0);assert.ok(restored.pet.moves.includes(52));assert.ok(restored.pet.moves.length<=4);
});
