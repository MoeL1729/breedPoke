...
function renderEgg(){
 $('hatch-odds').textContent=hatchOddsText(db);
...
async function hatchEgg(){
  ...
  modal(`<div class="hatch-reveal"><p class="eyebrow">${game.pet.shiny?'★ SHINY FRIEND':[132,133,147].includes(game.pet.speciesId)?`RARE FRIEND · ${Number((db.hatchTable.find(row=>row.id===game.pet.speciesId).weight/db.hatchTable.reduce((n,row)=>n+row.weight,0)*100).toFixed(4))}%`:'HELLO, LITTLE FRIEND'}</p><img class="pixel" src="${petSprite(game.pet.speciesId,false,game.pet.shiny)}" alt="${esc(game.species.name)}"><h2 class="modal-title">${esc(game.species.name)}가 태어났어요!</h2><p class="modal-copy">이제 이 친구와 함께 먹고, 쉬고, 모험해요.<br>둘만의 첫날을 시작해 볼까요?</p><button class="primary" id="meet-pet">함께 시작하기</button></div>`,'A NEW FRIEND');
