import { Game, freshState, migrateState, validateState, statsFor, clamp } from './engine.js?v=pokegotchi-4';
...
function renderBattle(){const b=game.s.battle;if(!b)return;$('pet').src=petSprite(b.player.transformedSpeciesId??b.player.speciesId,true);$('pet').alt=db.pokemon[b.player.transformedSpeciesId??b.player.speciesId].name;$('enemy').src=petSprite(b.enemy.transformedSpeciesId??b.enemy.speciesId);$('enemy').alt=db.pokemon[b.enemy.transformedSpeciesId??b.enemy.speciesId].name;$('enemy-card').innerHTML=hpCard(b.enemy);$('battle-player-hp').innerHTML=hpCard(b.player);
...
function confirmStone(to){
 ...
 modal(`<h2 class="modal-title">${esc(db.pokemon[e.to].name)}로 진화할까요?</h2><div class="evolve-art"><img class="pixel" src="${petSprite(game.pet.speciesId)}" alt=""><span>→</span><img class="pixel" src="${petSprite(e.to)}" alt=""></div><p class="modal-copy">${trade?`300 P로 ${e.heldItemName?esc(e.heldItemName)+'을 준비해 지니게 하고 ':''}NPC와 교환합니다. 같은 친구를 진화한 모습으로 돌려받습니다.${e.heldItemName?' 도구는 소모됩니다.':''} 실제 사용자 간 통신 교환은 아닙니다.`:`300 P로 ${esc(e.itemName)}을 구매해 사용합니다.`}<br>진화는 되돌릴 수 없어요. 기술표를 먼저 확인해 보세요.</p><div class="modal-actions"><button id="confirm-stone" class="primary" ${game.s.coins<300?'disabled':''}>진화 진행 · 300 P</button><button id="cancel-stone" class="secondary">나중에</button></div>`,label);
...
async function hatchEgg(){
  ...
  modal(`<div class="hatch-reveal"><p class="eyebrow">${game.pet.speciesId===147?'RARE FRIEND · 5%':game.pet.speciesId===133?'RARE FRIEND · 5%':'HELLO, LITTLE FRIEND'}</p><img class="pixel" src="${petSprite(game.pet.speciesId)}" alt="${esc(game.species.name)}"><h2 class="modal-title">${esc(game.species.name)}가 태어났어요!</h2><p class="modal-copy">이제 이 친구와 함께 먹고, 쉬고, 모험해요.<br>둘만의 첫날을 시작해 볼까요?</p><button class="primary" id="meet-pet">함께 시작하기</button></div>`,'A NEW FRIEND');
