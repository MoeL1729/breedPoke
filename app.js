import {abilityNotes} from './abilities.js?v=pokegotchi-250';
import { Game, freshState, migrateState, validateState, statsFor, clamp, ITEMS, shopCatalog, captureChance, SHINY_CHANCE, medicineUsable, TRAINERS, abilityOf, abilityItemTarget, HIDDEN_CHANCE, DROP_CHANCE } from './engine.js?v=pokegotchi-250';
...
const itemSymbol=id=>ITEMS[id]?.kind==='ability'?'★':ITEMS[id]?.kind==='ball'?'◉':ITEMS[id]?.kind==='medicine'?'✚':id==='quick-claw'?'爪':id==='leftovers'?'◒':id==='oran-berry'?'●':'◆';
...
function showBag(onlyBalls=false){
 ...
 return `<article class="shop-item"><span class="item-symbol">${itemSymbol(id)}</span><h3>${esc(i.name)} ×${game.s.inventory[id]}</h3><p>${esc(i.description)}</p>${chance!==null?`<p>포획 성공률 ${(chance*100).toFixed(1)}%</p>`:''}<button data-use="${id}" ${!(canBall||canHeal||canEquip||abilityTarget)?'disabled':''}>${i.kind==='ball'?'던지기':i.kind==='held'?'지니게 하기':i.kind==='ability'?(abilityTarget?esc(db.abilities[abilityTarget.id].name)+'(으)로 변경':'변경 불가'):i.kind==='medicine'?'사용하기':'진화 화면에서 사용'}</button></article>`;
