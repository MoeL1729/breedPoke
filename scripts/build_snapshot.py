...
# Include every first form whose complete three-stage chain is in this snapshot.
targets={e['to'] for p in pokemon.values() for e in p['evolutions']}
THREE_STAGE_ROOTS=[i for i in IDS if i not in targets and any(pokemon[e['to']]['evolutions'] for e in pokemon[i]['evolutions'])]
STARTERS=list(dict.fromkeys(PREVIOUS_STARTERS+THREE_STAGE_ROOTS))
COMMON_STARTERS=[i for i in STARTERS if i not in [147,133]]
HATCH_TABLE=[{'id':i,'weight':90} for i in COMMON_STARTERS]+[{'id':147,'weight':5*len(COMMON_STARTERS)},{'id':133,'weight':5*len(COMMON_STARTERS)}]
